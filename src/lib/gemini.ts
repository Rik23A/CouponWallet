...
Extract coupon details from OCR text and return ONLY a valid JSON object.
...
- expiryDate: ISO 8601 format "YYYY-MM-DD" (null if not found).
...
If a field cannot be determined, use null. Never guess couponCode — if unclear, use null.`;
...
export async function extractCouponFromOCR(rawOcrText: string, retryCount = 0): Promise<GeminiParsedCoupon | null> {
  if (!ai) return null;

  // Prioritize Gemma 4 31B for its reasoning capabilities.
  ...
  try {
    const prompt = `Extract coupon data from this OCR text:\n\n${rawOcrText}`;
    ...
    // Clean up response: sometimes models wrap JSON in markdown or add extra text
    ...
    if (isRetryable && retryCount < modelsToTry.length + 1) {
      ...
      return extractCouponFromOCR(rawOcrText, retryCount + 1);
    }
